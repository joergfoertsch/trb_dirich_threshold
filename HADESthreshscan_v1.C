...
void set_thresholds(std::shared_ptr<dirich> dirichptr, double thrinmV=30.)
{
	if(dirichptr==0){
		...
		if(thrinmV<0.){
			std::cerr << "negative thresholds are not \"allowed\"!\ninverting value" << std::endl;
			thrinmV = -1*thrinmV;
		}
	...
	if(search_dirich){
		...
		for(int i=0;i<ret;i+=4){
			...
			if(dirichlist.at(uint16_t(buffer[i+3]))->gMeasureTime!=.3){ //pls change it according to your initialization... Sure one should rather throw during init... but well I am lazy
				std::cerr << "DiRICH 0x" << std::hex << uint16_t(buffer[i+3]) << " not correclty initialized. Deleting!" << std::endl;
...
// implicit_value(std::vector<std::string>{"0"},"0")
		...
		if(vm["set-threshold"].empty() || (vm["set-threshold"].as<std::vector<std::string>>()).size() < 2){
			...
			for(auto& one_set_threshold_opt : each_set_threshold_opt){
				std::cout << "Setting Threshold of: "
									...
									<< std::endl;
				set_thresholds(one_set_threshold_opt.at(0) == "0" ? 0 : dirichlist.at(std::stoi(one_set_threshold_opt.at(0).substr(one_set_threshold_opt.at(0).find("0x")!=std::string::npos ? one_set_threshold_opt.at(0).find("0x")+2 : 0),NULL,16)), std::stoi(one_set_threshold_opt.at(1)));
			}
		}
	}
	...
	if(vm.count("set-pattern")){
		if(vm["set-pattern"].empty() || (vm["set-pattern"].as<std::vector<std::string>>()).size() < 2){
			std::cout << "no or less than two arguments were provided for option --set-pattern:\nno thresholds will be set" << std::endl;
		}
		else{
			...
			for(auto& one_set_pattern_opt : each_set_pattern_opt){
				std::cout << "Setting Threshold of: "
									<< std::stoi(one_set_pattern_opt.at(0).substr(one_set_pattern_opt.at(0).find("0x")!=std::string::npos ? one_set_pattern_opt.at(0).find("0x")+2 : 0),NULL,16) << "\t"
									<< " to: ";
									for(int ichannel=0;ichannel<NRCHANNELS;++ichannel) std::cout << (std::stol(one_set_pattern_opt.at(1)) >> ichannel) % 2;
									std::cout << std::endl;
				set_pattern(one_set_pattern_opt.at(0) == "0" ? 0 : dirichlist.at(std::stoi(one_set_pattern_opt.at(0).substr(one_set_pattern_opt.at(0).find("0x")!=std::string::npos ? one_set_pattern_opt.at(0).find("0x")+2 : 0),NULL,16)), std::stol(one_set_pattern_opt.at(1)));
			}
		}
	}
